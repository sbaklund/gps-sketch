 ...
 * routes/strava.js
 *
 * Strava OAuth + activity proxy, ported from the GPX Sketch Abstract app's
 * standalone server.js into a mountable Express Router so ONE backend
 * serves both terrain and Strava — no second service to deploy or pay for.
 *
 * Endpoints (unchanged contract from the original server):
 ...
 *   GET  /healthz           → { ok, mode }  (kept for parity with the original)
 *
 * KEY DIFFERENCE from the original: this app's front end lives on a
 * different domain (GitHub Pages) than this backend (Render). The original
 * server redirected back to '/' because it served the front end itself from
 * the same domain. Here, /auth/callback must redirect to FRONTEND_URL
 * instead — otherwise the person ends up looking at the bare backend after
 * signing in. Set FRONTEND_URL in the environment to your GitHub Pages URL.
 *
 * Strava's app settings ("Authorization Callback Domain") must be set to
 * THIS backend's domain (e.g. topo-backend-xxxx.onrender.com) — not the
 * GitHub Pages domain — since /auth/callback lives here.
 */
...
// Where the browser lands AFTER Strava sign-in completes — the front end's
// URL (GitHub Pages), a different domain than this backend. Falls back to
// BASE_URL for local single-domain testing.
